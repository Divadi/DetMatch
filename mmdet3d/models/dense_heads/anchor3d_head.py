...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def get_bboxes_raw(self,
                       ...
                       cfg=None,
                       rescale=False):
        """The idea is to mimic get_bboxes, but do not detach, and return the
        class probability distribution vectors for the bboxes. Basically, just
        decode. Note that this returns ALL boxes, so filtering is probably
        necessary.
        Args:
            cls_scores (list[torch.Tensor]): Multi-level class scores.
            bbox_preds (list[torch.Tensor]): Multi-level bbox predictions.
        ...
        for img_id in range(len(input_metas)):
            ...
            input_meta = input_metas[img_id]
            proposals = self.get_bboxes_raw_single(cls_score_list,
                                                   bbox_pred_list,
                                                   dir_cls_pred_list,
    ...
    def get_bboxes_raw_single(self,
                              cls_scores,
                              bbox_preds,
                              dir_cls_preds,
                              mlvl_anchors,
        ...
        # mlvl_bboxes = input_meta['box_type_3d'](
        #     mlvl_bboxes, box_dim=self.box_code_size)
        scores = torch.cat(mlvl_scores)
        dir_scores = torch.cat(mlvl_dir_scores)
