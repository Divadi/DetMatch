...
class HybridOptimizerConstructor(object):
    ...
    def __init__(self, optimizer_cfg, paramwise_cfg=None):
        ...
        self.base_lr = {
            x: optimizer_cfg[x].get('lr', None)
            for x in optimizer_cfg
        }
    ...
    def __call__(self, model):
        ...
        for name, param in model.named_parameters():
            ...
            for key in keys_prefix:
                if key in name:
                    # if the parameter match one of the custom keys, ignore
                    # other rules
                    for custom_key in sorted_keys:
                        if custom_key in name:
                            lr_mult = custom_keys[custom_key].get(
                                'lr_mult', 1.)
                            param_group['lr'] = self.base_lr[key] * lr_mult
                            logger.info(
                                f'learning rate of {name} is decreased by '
                                f'{lr_mult}')
                            break
