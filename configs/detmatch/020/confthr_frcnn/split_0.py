...
work_dir = outputs_dir + '{}/confthr_frcnn/split_{}'.format(pretrained_split_frac, split_num)
...
train_lab_info_path = data_root + '{}/kitti_infos_train_proj_3d_lab_{:.02f}_{}.pkl'.format(split_folder, split_frac, split_num)
train_unlab_info_path = data_root + '{}/kitti_infos_train_unlab_{:.02f}_{}.pkl'.format(split_folder, split_frac, split_num)
...
model = dict(
    ...
    ssl_cfg=dict(
        ...
        unlabeled=[
            ...
            dict( # (2D) Teacher: NMS
                type='BboxesNMS_2D',
                nms_cfg=dict(nms_pre=-1, score_thr=0.7, max_num=100, iou_thr=0.5),
            ...
            dict( # (2D) Teacher: Add student augs
                ...
                in_bboxes_key='tea.2d_bboxes_nms_no_aug', out_bboxes_key='tea.2d_bboxes_nms_stu_aug'),
            dict( # (2D) Teacher: Detach
                type='DetachBboxes',
                in_bboxes_key='tea.2d_bboxes_nms_stu_aug', out_bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch'),
            dict( # (2D) HardPseudoLabel cls_loss
                ...
                ssl_obj_attr='student.detector_2d', target_bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch',
                target_img_key='stu.img', target_img_metas_key='stu.img_metas',
                name='hard_pseudo_2d', weight=1),

            # Just for visualization
            dict( # Filter via score_thr
                type='MaxScoreFilter',
                cls_includes_bg_pred=True,
                score_thr=0.7, # this is also for recording metrics
                in_bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch', out_bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch_filt'),

            dict(
                type='NumPreds',
                bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch_filt', out_name='num_tea'),
            
            
            dict(
                type='Vis2D_Kitti',
                class_names=class_names, batch_dict_key='stu',
                tea_bboxes_key='tea.2d_bboxes_nms_stu_aug_dtch_filt', stu_bboxes_key=None,
                vis_idxs=train_unlab_info_path, vis_idxs_interval=50,
                out_name_prefix='Vis_2D_Tea'),
        ]),
